...
- 每台已保存服务器都提供折叠式 SSH Key 引导：可复用现有密钥或生成独立 Ed25519 密钥，只部署公钥，强制验证所选私钥，并在失败时回滚本次新增内容。
...
- 收藏服务器、复制 SSH 命令、打开系统终端、单独暂停服务器监控。“复制 SSH”会在能够静态确认时完整包含 `HostName`、`User` 和 `Port`，同时保留原配置文件的 `-F` 与别名；遇到条件式或动态配置时，会复制安全的别名命令并明确提示，而不会猜测连接参数。
...
- SSH Key 引导不会覆盖任何现有本地密钥。公钥通过 SSH 标准输入传送；远端会检查 `.ssh` / `authorized_keys` 的文件类型、所有者、权限和重复项，并在保存前用 `IdentitiesOnly=yes` 验证确实是所选私钥登录成功。
...
| 自动更新 | 启动后检查并提醒，不自动安装 | 启动后检查并提醒，不自动安装 |
